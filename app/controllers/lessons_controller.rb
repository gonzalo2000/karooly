class LessonsController < ApplicationController
  ...
  helper_method :expositions_complete?
  def expositions_complete?
    lesson ||= Lesson.find(params[:id])
    if current_enrollment = lesson.enrollment_for(current_user)
      current_enrollment.expositions_completed
    end
  end

  helper_method :unscrambled_complete?
  def unscrambled_complete?
    lesson ||= Lesson.find(params[:id])
    if current_enrollment = lesson.enrollment_for(current_user)
      current_enrollment.unscrambled_completed
    end
  end

  helper_method :dictation_complete?
  def dictation_complete?
    lesson ||= Lesson.find(params[:id])
    if current_enrollment = lesson.enrollment_for(current_user)
      current_enrollment.dictation_completed
    end
  end

  helper_method :image_spellings_complete?
  def image_spellings_complete?
    lesson ||= Lesson.find(params[:id])
    if current_enrollment = lesson.enrollment_for(current_user)
      current_enrollment.image_spelling_completed
    end
  end
  ...
  #this helper method will be phased out
